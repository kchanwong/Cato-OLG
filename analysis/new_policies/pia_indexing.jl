...
function project_economy_indexed(
    ...
    # Build COLA price series (independent from GDP inflation if cola_inflation is set)
    cola_inf_vec = isnothing(cola_inflation) ? copy(inf_vec) : fill(Float64(cola_inflation), n_years)
    ...
    if !isnothing(scenario_periods)
        for ep in scenario_periods
            ...
            @printf("  Scenario episode %d-%d: g_A=%.3f inf=%.3f cola_inf=%.3f\n",
                    ep[:year_start], ep[:year_end],
                    get(ep,:g_A,g_A), get(ep,:inflation,inflation),
                    get(ep,:cola_inflation, isnothing(cola_inflation) ? inflation : Float64(cola_inflation)))
        end
    end
    ...
    cum_cola_price     = cumprod([1.0; (1.0 .+ cola_inf_vec[2:end])])
    cum_cola_price_lag = [1.0; cum_cola_price[1:end-1]]
    ...
    for t in 1:n_years
        ...
        # --- PIA factor indexing: factors decay by price/wage ratio ---
    ...
    (year=year_cal, t=1:n_years,
     ...
     g_A_annual=g_A_vec, inflation_annual=inf_vec, cola_inflation_annual=cola_inf_vec,
